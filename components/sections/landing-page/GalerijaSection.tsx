...
import { getLatestProjects, getProjectBySlug } from '@/lib/projects'
...
export async function GalerijaSection() {
  const [cases, featuredProject] = await Promise.all([
    getLatestProjects(2, 'petrokov-andeo'),
    getProjectBySlug('petrokov-andeo'),
  ])
  ...
  return (
    <section id="galerija" className="py-[clamp(64px,8vw,120px)] px-[5%] bg-section-bg" aria-labelledby="galerija-title">
      ...
      <div className="max-w-[1100px] mx-auto flex flex-col gap-5">
        <RevealOnScroll>
          <article className="rounded-[20px] bg-dark overflow-hidden grid grid-cols-1 md:grid-cols-[1fr_clamp(240px,38%,400px)]">
            <div className="px-[clamp(32px,5vw,56px)] py-[clamp(36px,5vw,60px)]">
        ...
        {cases.length > 0 && (
          <RevealOnScroll delay={150}>
            <div className="grid grid-cols-1 sm:grid-cols-2 bg-white rounded-[20px] overflow-hidden divide-y sm:divide-y-0 sm:divide-x divide-[#e8e8e8]">
              {cases.map((project) => (
                <article key={project.id} className="flex flex-col sm:flex-row overflow-hidden">
                  <div className="p-[clamp(24px,3.5vw,44px)] flex flex-col flex-1">
                  ...
                  {project.cover_image && (
                    <div className="relative h-[200px] sm:h-auto sm:w-[180px] shrink-0">
                      <Image
